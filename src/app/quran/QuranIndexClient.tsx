      ...
      <div className="mt-8 grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(min(280px, 100%), 1fr))' }}>
        {filtered.map((c, index) => (
          <Link
            key={c.id}
            href={`/quran/${c.id}`}
            className="group rounded-xl glass-surface glass-interactive p-4 transition-all duration-300 ease-in-out hover:-translate-y-[1px] hover:scale-[1.02] animate-fade-in"
